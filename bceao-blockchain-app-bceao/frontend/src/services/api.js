...
const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:2002/api';
...
export const deactivateClient = async (ubi) => {
    try {
        const response = await api.delete(`/clients/${ubi}/deactivate`);
        return response.data;
    } catch (error) {
        throw new Error(error.response?.data?.error || 'Erreur lors de l\'activation du client');
    }
...
export const rejetDemande = async (ubi, demande_content) => {
    try {
        ...
        throw new Error(error.response?.data?.error || 'Erreur lors du rejet de la demande');
    }
};
