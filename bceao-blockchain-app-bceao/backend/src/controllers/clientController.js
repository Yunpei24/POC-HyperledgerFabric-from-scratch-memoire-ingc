...
class ClientController {
    ...
    // Rejet Activation/Désactivation un client
    async rejetDemande(req, res) {
        try {
            const { ubi } = req.params;
            const { demande_content } = req.body;  // Récupérer depuis le body au lieu des params
            
            const { contract } = await connectToNetwork();
            await contract.submitTransaction('RejetDemande', ubi, demande_content);
            
            res.json({ message: 'Rejet avec succès' });
        } catch (error) {
            console.error('Erreur rejetDemande:', error);
            res.status(500).json({ error: error.message });
        }
    }
